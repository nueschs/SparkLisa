...
spark_command_1 = 'spark-submit --class ch.unibnf.mcs.sparklisa.app.{0} --master yarn-client --num-executors {1} ../../../target/SparkLisa-0.0.1-SNAPSHOT.jar {2} {3} {4} {5} topology_bare_{6}_1600.txt {7}'
...
def collect_and_zip_output(log_file_name, num_base_stations, num_nodes, topology_type, run_type):
    ...
    shutil.copy('../resources/topology/topology_bare_{0}_{1}.txt'.format(topology_type, num_nodes), '../resources/temp/')
    list(hdfs_client.copyToLocal(['sparkLisa/results/{0}_{1}'.format(num_base_stations, num_nodes)+'/'], output_folder+'results/'))
    tar_name = '{0}_{1}_{2}_{3}_{4}_{5}_{6}'.format(run_type, num_base_stations, num_nodes, rate, window, duration, datetime.now().strftime(date_format))
...
def main():
    ...
    for num_base in [1]:
        spark_command_ = spark_command_1.format(
        ...
        p = subprocess.Popen(shlex.split('script '+log_file), stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=False)
